...
if __name__ == "__main__":
    ...
    '''
    with open(f'{dataset}_lidar_map_ownership.pkl', 'rb') as f:
        occupancy_overlap = pickle.load(f)
    
    print(len(list(occupancy_overlap.keys())))
    '''
    graph = gtsam.NonlinearFactorGraph()
    prior_model = gtsam.noiseModel.Diagonal.Sigmas((.01, .01, .01))
    initial_estimate = gtsam.Values()

    odometry_model = gtsam.noiseModel.Diagonal.Sigmas((.01, 0.01, .01))
    ...
    for t in tqdm(range(odom_sm.shape[0])):
        ...
        if t != odom_sm.shape[0] - 1:
            ...
            #                                      odom_sm[t+1,2] - odom_sm[t,2]), odometry_model))
            graph.add(Between(t, t+1, gtsam.Pose2(odom_sm[t+1,0] - odom_sm[t,0],
                                                  odom_sm[t+1,1] - odom_sm[t,1],
                                                  mat2euler(poses[t+1] @ np.linalg.inv(poses[t]))[2] ), odometry_model))
        if t!= 0 and t % n == 0:
            count += 1
        ...
        '''
        for j in range(t,odom_sm.shape[0]):
            if not occupancy_overlap[t].isdisjoint(occupancy_overlap[j]):
                ...
                graph.add(Between(t, j, gtsam.Pose2(odom_sm[j,0] - odom_sm[t,0],
                                                  ...
                                                  odom_sm[j,2] - odom_sm[t,2]), odometry_model))
        '''
